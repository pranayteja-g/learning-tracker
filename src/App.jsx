...
export default function LearningTracker() {
  ...
  const handleImport = (e) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        setNotes(data.notes || {});
        setImportFeedback({ ok: true, msg: "Data imported successfully!" });
      } catch {
        setImportFeedback({ ok: false, msg: "Invalid file — please use an exported tracker file." });
      }
  ...
  return (
    <div style={{ fontFamily: "'Georgia', serif", minHeight: "100vh", background: "#0f0f13", color: "#e8e6e0" }}>
          ...
          <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
            <button onClick={handleExport}
              style={{ padding: "6px 12px", border: "none", borderRadius: 6, cursor: "pointer",
                fontFamily: "inherit", fontSize: 12, background: "#1e1e24", color: "#888" }}>
              ⬇ Export
            </button>
      ...
      {/* DASHBOARD */}
      {view === "dashboard" && (
        <div style={{ padding: "28px", overflowY: "auto", maxHeight: "calc(100vh - 88px)" }}>
          {(() => { const t = totalStats(); return (
            <div style={{ background: "#16161b", border: "1px solid #2a2a3a", borderRadius: 12,
              padding: "22px 26px", marginBottom: 24, display: "flex", alignItems: "center", gap: 22 }}>
              <div style={{ position: "relative", width: 80, height: 80, flexShrink: 0 }}>
                <RadialProgress pct={t.pct} color="#7b5ea7" size={80} />
                <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center",
                  justifyContent: "center", fontSize: 14, fontWeight: 700, color: "#fff" }}>{t.pct}%</div>
              </div>
              <div>
                <div style={{ fontSize: 11, color: "#666", textTransform: "uppercase", letterSpacing: 1, marginBottom: 3 }}>Overall Progress</div>
                <div style={{ fontSize: 22, fontWeight: 700, color: "#fff" }}>{t.done} <span style={{ fontSize: 14, color: "#555", fontWeight: 400 }}>/ {t.total} topics</span></div>
                <div style={{ fontSize: 12, color: "#666", marginTop: 2 }}>across {Object.keys(roadmaps).length} roadmaps · {allNotes.length} notes written</div>
              </div>
            </div>
          ); })()}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(270px, 1fr))", gap: 14, marginBottom: 28 }}>
            {Object.entries(roadmaps).map(([key, val]) => {
              ...
              return (
                <div key={key} style={{ background: "#16161b", border: `1px solid ${val.color}33`, borderRadius: 12, padding: "18px 20px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 14 }}>
                    <div style={{ position: "relative", width: 60, height: 60 }}>
                      <RadialProgress pct={s.pct} color={val.color} size={60} />
                      <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center",
                        justifyContent: "center", fontSize: 12, fontWeight: 700, color: "#fff" }}>{s.pct}%</div>
                    </div>
                    <div>
                      <div style={{ fontSize: 14, fontWeight: 700, color: "#fff" }}>{val.label}</div>
                      <div style={{ fontSize: 12, color: val.accent, marginTop: 1 }}>{s.done} / {s.total} topics</div>
                    </div>
                  </div>
                  ...
                  {next.length > 0 && (
                    <div style={{ marginBottom: 12 }}>
                      <div style={{ fontSize: 10, color: "#555", textTransform: "uppercase", letterSpacing: 1, marginBottom: 5 }}>Up next</div>
                      {next.map(({ topic }) => (
                  ...
                  <button onClick={() => { setActiveRoadmap(key); setView("sections"); setActiveSection(null); }}
                    style={{ width: "100%", padding: "7px", background: val.color + "22", border: `1px solid ${val.color}44`,
                      borderRadius: 6, color: val.accent, fontSize: 12, cursor: "pointer", fontFamily: "inherit" }}>
                    Open →
                  </button>
                </div>
              );
          ...
          {allNotes.length > 0 && (
            <div>
              <div style={{ fontSize: 12, color: "#555", textTransform: "uppercase", letterSpacing: 1, marginBottom: 10 }}>📝 All Notes ({allNotes.length})</div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(250px, 1fr))", gap: 8 }}>
                {allNotes.map(([key, text]) => {
                  const [rmKey, topic] = key.split("::");
                  const val = roadmaps[rmKey];
                  return (
                    <div key={key} onClick={() => { setActiveRoadmap(rmKey); setView("sections"); setNoteDraft(text); setNoteModal({ roadmap: rmKey, topic }); }}
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "22px 26px" }}>
            {view === "nextup" && (
              <div>
                ...
                {nextUp.length === 0
                  ? <div style={{ color: rm.accent }}>🎉 Roadmap complete!</div>
                  : nextUp.map(({ section, topic }, i) => (
                    <div key={topic} style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 14px",
                      background: "#16161b", borderRadius: 8, marginBottom: 7, border: "1px solid #1e1e24" }}>
                      <div style={{ width: 24, height: 24, borderRadius: "50%", background: rm.color + "22",
                      ...
                      <div style={{ flex: 1, cursor: "pointer" }} onClick={() => toggle(activeRoadmap, topic)}>
                        <div style={{ fontSize: 13, color: "#e8e6e0" }}>{topic}</div>
                        <div style={{ fontSize: 11, color: "#555", marginTop: 1 }}>{section}</div>
                      </div>
                      {hasNote(activeRoadmap, topic) && (
                        <span style={{ fontSize: 10, color: rm.accent, background: rm.color + "22", padding: "2px 6px", borderRadius: 4 }}>note</span>
                      )}
                      <button onClick={e => openNote(e, activeRoadmap, topic)}
                        style={{ padding: "3px 8px", background: "transparent", border: "1px solid #2a2a35",
                          borderRadius: 4, color: "#666", fontSize: 11, cursor: "pointer", fontFamily: "inherit" }}>✏️</button>
                      <div onClick={() => toggle(activeRoadmap, topic)}
                        style={{ width: 17, height: 17, borderRadius: 4, border: `2px solid ${rm.color}`,
                          background: isDone(activeRoadmap, topic) ? rm.color : "transparent",
                          display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0 }}>
                        {isDone(activeRoadmap, topic) && <span style={{ color: "#fff", fontSize: 10 }}>✓</span>}
                      </div>
                    </div>
                  ))
                }
            ...
            {view === "sections" && (
              <div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                  {rm.sections[currentSection]?.map(topic => {
                    ...
                    return (
                      <div key={topic} style={{ background: done ? rm.color + "12" : "#16161b", borderRadius: 8,
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 13px", cursor: "pointer" }}
                          ...
                          <div style={{ width: 17, height: 17, borderRadius: 4, border: `2px solid ${done ? rm.color : "#444"}`,
                            background: done ? rm.color : "transparent", display: "flex", alignItems: "center",
                            justifyContent: "center", flexShrink: 0, transition: "all 0.15s" }}>
                            {done && <span style={{ color: "#fff", fontSize: 10 }}>✓</span>}
                          </div>
                          <span style={{ flex: 1, fontSize: 13, color: done ? rm.accent : "#ccc",
                            textDecoration: done ? "line-through" : "none", opacity: done ? 0.7 : 1 }}>{topic}</span>
                          {hasNote(activeRoadmap, topic) && (
                            <span style={{ fontSize: 10, color: rm.accent, background: rm.color + "20", padding: "2px 6px", borderRadius: 4 }}>note</span>
                          )}
                          <button onClick={e => openNote(e, activeRoadmap, topic)}
                            style={{ padding: "3px 8px", background: "transparent", border: "1px solid #2a2a35",
                              borderRadius: 4, color: "#555", fontSize: 11, cursor: "pointer", fontFamily: "inherit" }}>✏️</button>
                        </div>
                        {note.trim() && (
                          <div style={{ padding: "6px 13px 9px 40px", fontSize: 11, color: "#666", fontStyle: "italic",
        ...
        <div onClick={() => setNoteModal(null)}
          ...
          <div onClick={e => e.stopPropagation()}
            style={{ background: "#16161b", border: "1px solid #2a2a35", borderRadius: 12,
              padding: "22px", width: 450, maxWidth: "90vw", boxShadow: "0 20px 60px rgba(0,0,0,0.6)" }}>
            <div style={{ marginBottom: 12 }}>
            ...
            <textarea ref={textareaRef} value={noteDraft} onChange={e => setNoteDraft(e.target.value)}
              ...
              style={{ width: "100%", minHeight: 110, background: "#0f0f13", border: "1px solid #2a2a35",
                borderRadius: 7, padding: "9px 11px", color: "#e8e6e0", fontSize: 13, fontFamily: "inherit",
                resize: "vertical", outline: "none", boxSizing: "border-box", lineHeight: 1.6 }} />
            <div style={{ display: "flex", gap: 7, marginTop: 10, justifyContent: "flex-end" }}>
              <button onClick={() => setNoteModal(null)}
                style={{ padding: "6px 14px", background: "transparent", border: "1px solid #2a2a35",
                  borderRadius: 6, color: "#666", fontSize: 12, cursor: "pointer", fontFamily: "inherit" }}>Cancel</button>
              {getNote(noteModal.roadmap, noteModal.topic) && (
                <button onClick={deleteNote}
                  style={{ padding: "6px 14px", background: "#1f1212", border: "1px solid #3a2020",
                    borderRadius: 6, color: "#e05252", fontSize: 12, cursor: "pointer", fontFamily: "inherit" }}>Delete</button>
              )}
              <button onClick={saveNote}
                style={{ padding: "6px 14px", background: roadmaps[noteModal.roadmap]?.color,
                  border: "none", borderRadius: 6, color: "#fff", fontSize: 12, cursor: "pointer", fontFamily: "inherit" }}>Save Note</button>
            </div>
          </div>
        </div>
      )}
