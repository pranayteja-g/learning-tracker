...
import { allTopicNames } from "../../utils/topics.js";
    ...
    >
      ...
      {actions.map((action, i) => (
        <div key={i} style={{ fontSize: 12, color: "#52b788", lineHeight: 1.6 }}>
          {action.success ? "OK" : "ERR"} {action.message || action.error}
        </div>
      ))}
    </div>
  );
...
export function SagePanel({ open, onClose, appContext }) {
  ...
  const roadmapLabels = useMemo(
    () => Object.values(roadmaps).map((r) => r.label).join(", "),
    [roadmaps]
  );
      ...
      setMessages([
        {
          role: "assistant",
          content:
            `Hi! I'm Sage - your learning mentor.\n\n` +
            `I can see your roadmaps: ${roadmapLabels || "none yet"}\n\n` +
            `I can help you:\n` +
            `- Answer questions about any topic\n` +
            `- Save notes directly to your roadmap\n` +
            `- Read images and extract useful knowledge\n` +
  ...
  }, [open, messages.length, roadmapLabels]);

  const buildSystemPrompt = () => {
    const rmSummary = Object.values(roadmaps)
      .map((rm) => {
        const topics = Object.values(rm.sections).flatMap(allTopicNames);
...
- When reading images: extract the useful information clearly and suggest where it should be saved
- Be proactive: if the user shares an image of notes, offer to save them
...
- Use mark_topic_done when user says they've learned or finished something
...
Always respond in a helpful, mentor-like tone. Format responses with markdown.`;
  ...
  const send = async () => {
    ...
    setError("");
    const userMsg = { role: "user", content: text || "Please analyze this image.", image: image?.preview };
    ...
    try {
      ...
      const currentContent = image
        ? [
            { type: "text", text: text || "Please analyze this image and extract useful information." },
      ...
      if (msg?.tool_calls?.length) {
        ...
        for (const toolCall of msg.tool_calls) {
          ...
          const result = executeTool(toolCall.function.name, args, appContext);
          actions.push(result);
          toolResults.push({
            role: "tool",
        ...
        const followUp = await fetch("/api/nvidia", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
        ...
        if (!followUp.ok) {
          ...
          throw new Error(err.error?.message || `API error ${followUp.status}`);
        }
        ...
        setMessages((prev) => [
          ...prev,
          { role: "assistant", content: msg?.content || "...", actions },
        ]);
      }
    } catch (e) {
      setMessages((prev) => [
        ...prev,
        { role: "assistant", content: `Sorry, I ran into an issue: ${e.message}` },
      ]);
    } finally {
      setLoading(false);
  ...
  return (
    ...
    >
      ...
      {!hasNvidiaKey && (
        ...
        >
          Add your <strong>NVIDIA NIM API key</strong> in Settings -&gt; AI to use Sage. Get a free key at{" "}
          <a
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "16px" }}>
        {messages.map((msg, i) => (
          <Message key={i} msg={msg} color={color} />
        ))}
        ...
        <div style={{ padding: "8px 16px 0", flexShrink: 0 }}>
          <div style={{ position: "relative", display: "inline-block" }}>
            <img
              src={image.preview}
              alt=""
              style={{ height: 60, borderRadius: 8, border: `1px solid ${color}44`, objectFit: "cover" }}
            />
            <button
