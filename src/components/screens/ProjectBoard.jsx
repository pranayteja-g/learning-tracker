  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
        {milestones.map((m, i) => (
          <div key={i} onClick={() => onToggle(rmId, projId, i)}
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 12, color: m.done ? "#52b788" : "#ccc",
                textDecoration: m.done ? "line-through" : "none",
                textDecorationColor: "#52b78866" }}>{m.title || m}</div>
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      <div style={{ padding: "12px 16px", borderBottom: "1px solid #1e1e24",
        ...
        <button onClick={onBack} style={{ background: "transparent", border: "none",
          color: "#666", fontSize: 20, cursor: "pointer", padding: 0, lineHeight: 1 }}>‹</button>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: "#fff",
            overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
            {project.title}
          </div>
          <div style={{ fontSize: 11, color: sc.color }}>{sc.icon} {sc.label}</div>
        </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "16px" }}>
        ...
        <div style={{ display: "flex", gap: 6, marginBottom: 16 }}>
          {Object.entries(STATUS_CONFIG).map(([s, cfg]) => (
            <button key={s} onClick={() => onSetStatus(s)}
              style={{ flex: 1, padding: "7px 4px", background: project.status === s ? cfg.bg : "transparent",
                border: `1px solid ${project.status === s ? cfg.color : "#2a2a35"}`,
                borderRadius: 7, color: project.status === s ? cfg.color : "#444",
        ...
        {/* Auto-suggest complete if all milestones done */}
        {allDone && project.status !== "completed" && (
          <div style={{ background: "#1a2e1a", border: "1px solid #52b78844",
            borderRadius: 8, padding: "10px 14px", marginBottom: 14,
            display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <span style={{ fontSize: 12, color: "#52b788" }}>🎉 All milestones done!</span>
            <button onClick={() => onSetStatus("completed")}
              style={{ padding: "5px 12px", background: "#52b788", border: "none",
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 5, marginBottom: 16 }}>
          ...
          {project.topics?.map(t => (
            <span key={t} style={{ fontSize: 10, padding: "2px 7px", borderRadius: 4,
              background: rm.color + "1a", color: rm.accent, border: `1px solid ${rm.color}33` }}>
              {t}
            </span>
          ))}
        </div>
          ...
          <div style={{ marginBottom: 16 }}>
            <button onClick={() => setShowHints(h => !h)}
              style={{ width: "100%", padding: "9px 12px", background: "#0f0f13",
                border: "1px solid #2a2a35", borderRadius: 8, cursor: "pointer",
                fontFamily: "inherit", display: "flex", justifyContent: "space-between",
                alignItems: "center", marginBottom: showHints ? 8 : 0 }}>
              <span style={{ fontSize: 12, color: "#888" }}>💡 Hints ({project.hints.length})</span>
              <span style={{ fontSize: 11, color: "#444" }}>{showHints ? "▲" : "▼"}</span>
            </button>
            {showHints && project.hints.map((h, i) => (
              <div key={i} style={{ padding: "10px 12px", background: "#1a1a2e",
                border: "1px solid #7b5ea733", borderRadius: 7, marginBottom: 5,
        ...
        {project.links?.length > 0 && (
          <div style={{ marginBottom: 16 }}>
            <div style={{ fontSize: 11, color: "#555", textTransform: "uppercase",
              letterSpacing: 1, marginBottom: 8 }}>Resources</div>
      ...
      {showDelete && (
        <div style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.7)",
          ...
          <div style={{ background: "#16161b", border: "1px solid #2a2a35", borderRadius: 12,
            padding: "20px", width: "100%", maxWidth: 300, textAlign: "center" }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => setShowDelete(false)}
                style={{ flex: 1, padding: "9px", background: "#1e1e24", border: "1px solid #2a2a35",
                  borderRadius: 7, color: "#888", fontSize: 13, cursor: "pointer", fontFamily: "inherit" }}>
                Cancel
              </button>
    ...
    return (
      <div style={{ position: "fixed", inset: 0, background: "#0f0f13", zIndex: 200,
        ...
        <ProjectDetail
          ...
          onDelete={() => { onDelete(rm.id, proj.id); setSelected(null); }}
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#0f0f13", zIndex: 200,
      ...
      <div style={{ padding: "14px 16px", borderBottom: "1px solid #1e1e24",
        display: "flex", alignItems: "center", gap: 10, background: "#13131a", flexShrink: 0 }}>
      ...
      <div style={{ display: "flex", borderBottom: "1px solid #1e1e24",
        ...
        {[["all","All"], ["inprogress","🔨 Active"], ["idea","💡 Ideas"], ["completed","✅ Done"]].map(([v,l]) => (
          <button key={v} onClick={() => setFilter(v)}
            style={{ flex: 1, padding: "9px 4px", border: "none", background: "transparent",
              fontSize: 11, fontWeight: filter === v ? 700 : 400,
          ...
          filtered.map(p => {
            ...
            return (
              <div key={p.id} onClick={() => setSelected(p.id)}
                style={{
                  background: p.status === "completed" ? "#0f1f0f"
                    : p.status === "inprogress" ? "#1c1800"
                    : "#16161b",
                  border: `1px solid ${p.status === "completed" ? "#52b78855"
                    : p.status === "inprogress" ? "#ee9b0055"
                    : "#2a2a35"}`,
                  borderLeft: `3px solid ${p.status === "completed" ? "#52b788"
                    : p.status === "inprogress" ? "#ee9b00"
                    : "#333"}`,
                  borderRadius: 10, padding: "13px 14px", marginBottom: 10,
                  cursor: "pointer", overflow: "hidden"
                }}>
                {/* Progress bar top — only for inprogress */}
                {mTotal > 0 && p.status === "inprogress" && (
                  <div style={{ height: 3, background: "#2a2a00", borderRadius: 2,
                    overflow: "hidden", marginBottom: 10 }}>
                    <div style={{ height: "100%", width: `${pct}%`,
                      background: "#ee9b00", borderRadius: 2, transition: "width 0.3s" }} />
                  </div>
                )}
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  <div style={{ flex: 1, minWidth: 0, marginRight: 10 }}>
                    <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 5,
                      color: p.status === "completed" ? "#52b788"
                        : p.status === "inprogress" ? "#fff" : "#aaa",
                    ...
                    <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}>
                      <span style={{ fontSize: 10, padding: "1px 6px", borderRadius: 3,
                        background: sc.bg, color: sc.color, fontWeight: 700 }}>{sc.label}</span>
                      <span style={{ fontSize: 10, color: "#444" }}>·</span>
                      <span style={{ fontSize: 10, color: p.difficulty === "beginner" ? "#52b788" : "#ee9b00",
                        textTransform: "uppercase", fontWeight: 700 }}>{p.difficulty}</span>
                      {mTotal > 0 && (
                        <>
                          <span style={{ fontSize: 10, color: "#444" }}>·</span>
                          <span style={{ fontSize: 10, color: "#666" }}>{mDone}/{mTotal} steps</span>
                        </>
                      )}
                      {p.estimatedTime && (
                        <>
