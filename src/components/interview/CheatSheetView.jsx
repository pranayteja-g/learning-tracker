    ...
    <div style={{ padding: "0 16px 20px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start",
        ...
        <div style={{ minWidth: 0, flex: 1 }}>
          <div style={{ fontSize: 15, fontWeight: 700, color: "#fff",
            wordBreak: "break-word", overflowWrap: "anywhere" }}>{data.title}</div>
          ...
          <div key={key} style={{ marginBottom: 16 }}>
            ...
            <div style={{ background: "#0f0f13", borderRadius: 8, border: `1px solid ${cfg.color}22`,
              ...
              {items.map((item, i) => (
                <div key={i} style={{
                  padding: "10px 13px",
                  borderBottom: i < items.length - 1 ? "1px solid #1e1e24" : "none",
                }}>
                  {/* Topic name on its own line */}
      ...
      {data.quickTips?.length > 0 && (
        <div style={{ marginBottom: 16 }}>
          ...
          <div style={{ background: "#0f0f13", borderRadius: 8, border: "1px solid #7b8cde22", overflow: "hidden" }}>
            {data.quickTips.map((tip, i) => (
              <div key={i} style={{ padding: "9px 13px",
                ...
                <span style={{ fontSize: 11, color: "#7b8cde", flexShrink: 0, fontWeight: 700, marginTop: 1 }}>{i+1}.</span>
                <span style={{ fontSize: 12, color: "#888", lineHeight: 1.6,
                  wordBreak: "break-word", overflowWrap: "anywhere", flex: 1, minWidth: 0 }}>{tip}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {data.redFlags?.length > 0 && (
        <div style={{ marginBottom: 8 }}>
          ...
          <div style={{ background: "#0f0f13", borderRadius: 8, border: "1px solid #e0525222", overflow: "hidden" }}>
            {data.redFlags.map((flag, i) => (
              <div key={i} style={{ padding: "9px 13px",
                ...
                <span style={{ fontSize: 12, color: "#888", lineHeight: 1.6,
                  wordBreak: "break-word", overflowWrap: "anywhere", flex: 1, minWidth: 0 }}>{flag}</span>
              </div>
            ))}
          </div>
        </div>
